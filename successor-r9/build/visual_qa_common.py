...
PDF_RELATIVE = "output/pdf/stacks-project-ko-kr-cumulative-r6-49-chapters.pdf"
BUILD_RECEIPT_RELATIVE = "receipts/R6_COMPONENT_AND_CUMULATIVE_BUILD.json"
...
EVIDENCE = ROOT / "evidence" / "visual-qa-r6"
...
FINAL_RECEIPT = ROOT / "receipts" / "R6_PAGE_COMPLETE_VISUAL_QA.json"
...
def root_path(relative_path: str) -> Path:
    ...
    try:
        ...
        raise RuntimeError(f"path escapes r6 root: {relative_path}") from exc
    return candidate
...
def wait_for_build_inputs(timeout_seconds: float, poll_seconds: float) -> None:
    """Wait only for the exact cumulative receipt/PDF pair, then bind atomically."""
    if timeout_seconds < 0:
        raise ValueError("wait timeout must be nonnegative")
    if poll_seconds <= 0:
        raise ValueError("poll interval must be positive")
    deadline = time.monotonic() + timeout_seconds
    while not (BUILD_RECEIPT.is_file() and PDF.is_file()):
        remaining = deadline - time.monotonic()
        if remaining <= 0:
            missing = [
                relative(path)
                for path in (BUILD_RECEIPT, PDF)
                if not path.is_file()
            ]
            raise RuntimeError(
                f"timed out after {timeout_seconds:g} seconds waiting for "
                f"the cumulative build inputs: {missing}"
            )
...
def bind_build() -> tuple[dict[str, object], dict[str, object], int, list[dict[str, object]]]:
    ...
    if not PDF.is_file():
        raise RuntimeError(
            "the r6 cumulative PDF does not exist yet; no PDF identity is assumed by this tooling"
        )
...
def self_test_common() -> None:
    ...
    for timeout_seconds, poll_seconds in ((-1, 1), (0, 0)):
        try:
            wait_for_build_inputs(timeout_seconds, poll_seconds)
        except ValueError:
            pass
        else:
            raise AssertionError("invalid wait parameters were accepted")
