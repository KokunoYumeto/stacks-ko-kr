% Generated mechanically from frozen input canon/ko-kr/integration-20260908-r10-p03-register/inputs/p11/ko/defos.tex.
...
이러한 $B'$를 (\ref{defos-equation-to-solve})의 {\it 해}라고 부른다.
...
$$
앞쪽과 뒤쪽이 (\ref{defos-equation-to-solve})의 해이면 다음이 성립한다.
\begin{enumerate}
...
(\ref{defos-equation-to-solve})의 해가 존재하면, 해의 동형류들의 집합은
$\Ext^1_B(\NL_{B/A}, N)$ 아래의 주동차 공간이다.
\end{lemma}

\begin{proof}
(\ref{defos-equation-to-solve})의 두 해 $B'_1$, $B'_2$가 주어지면
보조정리 \ref{defos-lemma-huge-diagram}에 의해 준동형 $B'_1\to B'_2$의
존재에 대한 장애 원소
...
\begin{proof}
이를 보이기 위해 (\ref{defos-equation-to-solve})이 다음 도표로 주어지는 경우에
앞의 결과들을 적용한다.
$$
\xymatrix{
0 \ar[r] & N \ar[r] & {?} \ar[r] & B \ar[r] & 0 \\
...
$$
그러면 이 보조정리는 보조정리 \ref{defos-lemma-choices}와 해 $N\oplus B$가
존재한다는 사실에서 따른다. 전단사 대응의 직접적인 구성은 다음 주를 보라.
\end{proof}

\begin{remark}
\label{defos-remark-extensions-of-algebras}
$A\to B$와 $N$은 보조정리 \ref{defos-lemma-extensions-of-algebras}에서와
...
$0\to N\to C'\to C\to0$.
\end{enumerate}
보조정리 \ref{defos-lemma-extensions-of-algebras}를 보라. 이때 $B\to C$와
...
$\NL_{B/A}\to\NL_{C/A}$를 사용한다. 보조정리 \ref{defos-lemma-huge-diagram}을
...
$$
여기에 보조정리 \ref{defos-lemma-extensions-of-algebras}와
\ref{defos-lemma-huge-diagram}의 증명에 나오는 구성들의 양립성을 사용한다.
...
보조정리 \ref{defos-lemma-extensions-of-algebras-functorial}에서와 같다고 하자.
...
\ref{defos-lemma-extensions-of-algebras-functorial}을 보이려면, 보조정리
\ref{defos-lemma-extensions-of-algebras}의 전단사 대응 아래에서 $B'_1$이
다음 준동형에 의한 $\xi$의 상에 대응하고
$\Ext^1_B(\NL_{B/A}, M) \to \Ext^1_B(\NL_{B/A}, N)$
또 $B'_2$가 다음 준동형에 의한 $\zeta$의 상에 대응함을 보이면 충분하다.
$\Ext^1_C(\NL_{C/A}, N) \to \Ext^1_B(\NL_{B/A}, N)$.
첫 번째 주장은 주 \ref{defos-remark-extensions-of-algebras}에서 동치류를
...
$C$의 표시이다. 주 \ref{defos-remark-extensions-of-algebras}와 거기에 적힌
...
\ref{defos-remark-extensions-of-algebras}에서 동치류를 구성한 방법을 풀어 쓰면,
...
(\ref{defos-equation-to-solve})에서와 같다고 하자. 보조정리
\ref{defos-lemma-extensions-of-algebras}에 의해 $A$를 $I$로 확대한 확대 $A'$에
...
\ref{defos-lemma-extensions-of-algebras}를 적용하면,
$\Ext^1_B(\NL_{B/A'},N)$의 원소 $\zeta$들이 $A'$-대수 확대
$0\to N\to B'\to B\to0$을 매개화함을 알 수 있다. $A'\to A\to B$와
$c:I\to N$에 보조정리
\ref{defos-lemma-extensions-of-algebras-functorial}을 적용하면, $c$와
$A\to B$에 양립하는 $A'$-대수 준동형 $A'\to B'$가 존재하는 것과
$\zeta$가 $\xi$로 가는 것은 동치이다. 이는 물론 $B'$가
(\ref{defos-equation-to-solve})의 해라는 것과 같은 말이다.
\end{proof}

\begin{remark}
\label{defos-remark-parametrize-solutions}
보조정리 \ref{defos-lemma-parametrize-solutions}의 상황에서는
$$
\Ext^1_A(\NL_{A/A'}, N) =
...
(\ref{defos-equation-to-solve})의 해가 존재하는 데 대한 장애 원소라고
결론 내릴 수 있을 것이다.
\end{remark}
...
(\ref{defos-equation-to-solve})의 해가 존재한다.
\end{lemma}
...
$$
는 $N=I\otimes_A B$인 경우의 (\ref{defos-equation-to-solve})의 해이다.
일반적인 경우는 주어진 준동형 $I\otimes_A B\to N$에 따른
밀어내기를 취하여 얻는다.
\end{proof}

\begin{proof}[두 번째 증명]
이 증명을 읽기 전에 주 \ref{defos-remark-parametrize-solutions}를 읽어 두자.
...
상황 \ref{defos-situation-morphism-thickenings}에서 준동형
...
상황 \ref{defos-situation-morphism-thickenings}에서 사상 $(f,f')$이 두꺼워짐의
엄격 사상인 것과 (\ref{defos-equation-morphism-thickenings})이 환 달린 공간의
...
1차 두꺼워짐이라 하자. 절 \ref{defos-section-thickenings-spaces}에서 도입한
...
$$
(\ref{defos-equation-extension})에서와 같은 준동형
...
(\ref{defos-equation-extension})이 하나라도 존재하면, 그러한 확대들의 동형류
...
(\ref{defos-equation-extension})이 존재하기 위한 필요충분조건이다.
\end{lemma}

\begin{proof}
먼저 $\mathcal{K}$가 단사 $\mathcal{O}_X$-가군이면
$c_{\mathcal{F}'}=c$인 완전열 (\ref{defos-equation-extension})이 실제로
...
0 \to \mathcal{K}' \to \mathcal{E}' \to \mathcal{F} \to 0
$$
이는 (\ref{defos-equation-extension})에서와 같고 $c_{\mathcal{E}'}=c'$이다.
...
$$
이는 (\ref{defos-equation-extension})에서와 같고 $c_{\mathcal{D}'}=0$이다.
...
보조정리 \ref{defos-lemma-inf-ext}와 단사 가군 $\mathcal{K}'$로 가는 고차
...
$$
$\pi$에 딸린 분할 (\ref{defos-equation-splitting})과 준동형 $c$를 사용하여
$\mathcal{O}_{X'}$-가군 구조를 정의하면 확대
(\ref{defos-equation-extension})을 얻는다. $\mathcal{F}'_{c,triv}$를 $c$와
자명화 $\pi$에 대응하는, $\mathcal{F}$를 $\mathcal{K}$로 확대한
{\it 자명한 확대}라고 부른다. (\ref{defos-equation-extension})과 같은 임의의
확대 $\mathcal{F}'$에 대하여
...
$\xi_{\mathcal{F}'}$를 얻는다. 보조정리 \ref{defos-lemma-inf-ext}에 의해
...
\begin{matrix}
\text{확대의 동형류}\\
\mathcal{F}'\text{는 (\ref{defos-equation-extension})과 같고 }c = c_{\mathcal{F}'}
\end{matrix}
\right\}
\longrightarrow
...
\left\{
\begin{matrix}
\mathcal{F}'_2\text{는 (\ref{defos-equation-extension})과 같고 }\\
...
\left\{
\begin{matrix}
\mathcal{F}'_1\text{는 (\ref{defos-equation-extension})과 같고 }\\
...
(X'_2,\mathcal{O}_{X'_2})$는 주 \ref{defos-remark-extension-functorial}에서와
...
$$
이 상황에서 주 \ref{defos-remark-trivial-extension}의 구성은 다음 가환
도표를 유도한다.
$$
\xymatrix{
\{\mathcal{F}'_2\text{는 (\ref{defos-equation-extension})과 같고 }
...
주 \ref{defos-remark-extension-functorial}에서 구성한 것이다.
\end{remark}
...
\ref{defos-remark-short-exact-sequence-thickenings}를 보라. 각 $i=1,2,3$에
...
(\ref{defos-equation-extension})과 같고 $c_{\mathcal{F}'_2}=c_2$이다.
주 \ref{defos-remark-extension-functorial}의 함자성을 적용하여 $X'_1$ 위의
확대 $\mathcal{F}'_1$을 얻을 수 있다. 이 특별한 경우의
$\mathcal{F}'_1$은 아래에서 설명한다. 주
\ref{defos-remark-trivial-extension}에 따라, 주
\ref{defos-remark-short-exact-sequence-thickenings}의 표준 자명화
...
$$
보조정리 \ref{defos-lemma-inf-obs-ext}를 보라. 이 상황에서 짧은 완전열
...
\ref{defos-lemma-inf-obs-ext}의 증명은 $o(\mathcal{F},\mathcal{K}_3,c_3)$를
...
(\ref{defos-equation-extension})과 같은 확대이다. 특히
...
\begin{proof}
이는 보조정리 \ref{defos-lemma-inf-map}의 특수한 경우이지만 직접 증명도
...
$(f, f')$를 상황 \ref{defos-situation-morphism-thickenings}에서와 같은
...
(정의 \ref{defos-definition-strict-morphism-thickenings}). 그러면 다음
조건들이 동치이다.
\begin{enumerate}
...
\begin{proof}
보조정리 \ref{defos-lemma-inf-map-special}과
\ref{defos-lemma-deform-module}을 결합하면 된다.
\end{proof}
...
\begin{proof}
보조정리 \ref{defos-lemma-inf-map-special}의 증명으로부터, 다음 준동형에
...
보조정리 \ref{defos-lemma-inf-obs-map-special}에서 따른다.
$$
\Ext^1_{\mathcal{O}_X}(Li^*\mathcal{F}',
\mathcal{I}\mathcal{G}') =
...
\mathcal{G} \otimes_{\mathcal{O}_X} f^*\mathcal{J})
$$
실제로 보조정리 \ref{defos-lemma-deform-module}에 의해
$\mathcal{I}\mathcal{G}' =
...
\ref{defos-lemma-deform-module}의 마지막 명제에 의해 영이다. 따라서
...
\begin{proof}[둘째 증명]
보조정리 \ref{defos-lemma-inf-obs-map}을 다음과 같이 적용할 수 있다.
보조정리 \ref{defos-lemma-deform-module}에 의해
...
\ref{defos-lemma-deform-module}에 의해 표준 준동형
$$
f^*\mathcal{J} \otimes_{\mathcal{O}_X} \mathcal{F} \to
\mathcal{I} \otimes_{\mathcal{O}_X} \mathcal{F}
$$
...
보조정리 \ref{defos-lemma-inf-ext}를 적용하자. 보조정리
\ref{defos-lemma-deform-module}에 의해 대응하는 확대 $\mathcal{F}'$는 모두
$S'$ 위에서 평탄하다.
\end{proof}
...
\mathcal{I} \otimes_{\mathcal{O}_X} \mathcal{F}$
은 동형사상이다.
\item 보조정리 \ref{defos-lemma-inf-obs-ext}의 동치류
...
\begin{proof}
이는 보조정리 \ref{defos-lemma-deform-module}에 주어진 $S'$ 위에서 평탄한
$\mathcal{O}_{X'}$-가군의 특징짓기와 보조정리
\ref{defos-lemma-inf-obs-ext}에서 바로 따른다.
\end{proof}
...
$$
가로 화살표들은 상황 \ref{defos-situation-morphism-thickenings}에서와 같은
...
(보조정리 \ref{defos-lemma-deform-module}을 $\mathcal{O}_{X'}$에 적용한다).
...
필요충분조건은 보조정리 \ref{defos-lemma-inf-obs-ext}의 동치류
...
\ref{defos-lemma-inf-obs-ext-rel}에서 따른다. (2)는 보조정리
\ref{defos-lemma-inf-ext-rel}에서, (3)은 보조정리
\ref{defos-lemma-inf-map-rel}에서 따른다.
\end{proof}
...
(주 \ref{defos-remark-short-exact-sequence-thickenings}).
\end{situation}

\begin{lemma}
\label{defos-lemma-verify-iv}
상황 \ref{defos-situation-ses-flat-thickenings}에서 가군
$\pi^*\mathcal{F}$와 $h^*\mathcal{F}'_2$는 $S'_1$ 위에서 평탄하고
$X$ 위로 제한하면 $\mathcal{F}$가 되는 $\mathcal{O}'_1$-가군이다.
이 둘의 차이(보조정리 \ref{defos-lemma-flat})는
$\Ext^1_{\mathcal{O}_X}(
...
올리는 데 대한 장애 원소(보조정리 \ref{defos-lemma-flat})와 같다.
\end{lemma}
...
따라서 보조정리 \ref{defos-lemma-deform-module}에 의해 평탄하다.
...
$$
이는 상황 \ref{defos-situation-ses-flat-thickenings}의 가정들과
$\mathcal{F}$가 $S$ 위에서 평탄하다는 사실에서 따른다. 장애 동치류에
대한 명제는 주 \ref{defos-remark-complex-thickenings-and-ses-modules}의
...
(\ref{defos-equation-morphism-thickenings})에서와 같은 두꺼워짐의 사상
$(f, f')$를 찾는다. 여기서 $\Ker(i^\sharp)$를 $\mathcal{G}$와
동일시하며 $(f')^\sharp$가 주어진 사상 $c$를 유도해야 한다.
이때 $X'$를 (\ref{defos-equation-to-solve-ringed-spaces})의 {\it 해}라고
부른다.
...
\end{equation}
이 도표의 앞면과 뒷면은 (\ref{defos-equation-to-solve-ringed-spaces})의
해이다.
\begin{enumerate}
\item 다음 군에는 표준 원소가 존재한다.
$\Ext^1_{\mathcal{O}_{X_2}}(Lg^*\NL_{X_1/S_1}, \mathcal{G}_2)$
이 원소가 영인 것은 $\nu$와 양립하며
(\ref{defos-equation-huge-1})에 들어맞는 환 달린 공간의 사상
$X'_2 \to X'_1$이 존재하기 위한 필요충분조건이다.
\item $\nu$와 양립하며 (\ref{defos-equation-huge-1})에 들어맞는 사상
...
$$
(\ref{defos-equation-huge-1})에 들어맞고 $\nu$와 양립하는 환 달린 공간의
...
(\ref{defos-equation-huge-2})가 주어졌고, 그 도표에 들어맞는 환층의 준동형
...
(\ref{defos-equation-huge-2})의 왼쪽 정사각형이 가환하므로
...
그때에만 $[\xi]$가 영이다. (\ref{defos-equation-huge-2})에 들어맞는 환층의
...
(\ref{defos-equation-huge-2})에 들어맞는 준동형
$\mathcal{O}_{X'_1} \to \mathcal{O}_{X'_2}$를 정한다.

\medskip\noindent
위의 특수한 경우에서 (2)의 증명. 생략한다. 도움말: 이는 보조정리
\ref{defos-lemma-huge-diagram}의 (2)의 증명과 완전히 같다.
\end{proof}
...
(\ref{defos-equation-to-solve-ringed-spaces})의 해가 존재하면, 해들의 동형류
집합은 $\Ext^1_{\mathcal{O}_X}(\NL_{X/S}, \mathcal{G})$ 아래의
주동차 공간이다.
\end{lemma}

\begin{proof}
(\ref{defos-equation-to-solve-ringed-spaces})의 두 해 $X'_1$과 $X'_2$가
주어지면, 보조정리 \ref{defos-lemma-huge-diagram-ringed-spaces}에 의해
...
\medskip\noindent
동치류 $\xi$에 대하여 보조정리
\ref{defos-lemma-NL-represent-ext-class}에서와 같이
...
\begin{proof}
이를 증명하기 위해, (\ref{defos-equation-to-solve-ringed-spaces})이 다음
...
보조정리 \ref{defos-lemma-choices-ringed-spaces}에서 따른다. 전단사의 직접
구성은 아래의 주를 보라.
\end{proof}
...
보조정리 \ref{defos-lemma-extensions-of-relative-ringed-spaces}에서와 같이
택하자. 보조정리에서와 같은 확대
$0 \to \mathcal{G} \to \mathcal{O}_{X'} \to \mathcal{O}_X \to 0$
...
\end{enumerate}
보조정리 \ref{defos-lemma-extensions-of-relative-ringed-spaces}를 보라.
...
\ref{defos-lemma-huge-diagram-ringed-spaces}을 다음 도표에 적용하면
보조정리의 명제를 얻을 수 있다.
$$
\xymatrix{
& 0 \ar[r] &
...
$$
여기에 보조정리 \ref{defos-lemma-extensions-of-relative-ringed-spaces}와
\ref{defos-lemma-huge-diagram-ringed-spaces}의 증명에 나오는 구성들 사이의
...
보조정리 \ref{defos-lemma-extensions-of-relative-ringed-spaces-functorial}에서와
...
\ref{defos-lemma-extensions-of-relative-ringed-spaces}에 의해 이 확대들은
다음 등식의 왼쪽 항으로 분류된다.
$$
...
\ref{defos-lemma-extensions-of-relative-ringed-spaces-functorial}을 보이려면
$\mathcal{O}'_1$이 $\xi$의 상에 대응하고 $\mathcal{O}'_2$가
$\zeta$의 상에 대응함을 보이면 충분하다. $\xi$와
$\mathcal{O}'_1$ 사이의 대응은 주
\ref{defos-remark-extensions-of-relative-ringed-spaces}에서 동치류 $\xi$를
...
\ref{defos-lemma-extensions-of-relative-ringed-spaces-functorial}의 명제에서
사용한 사상 $Lg^*\NL_{X/S} \to \NL_{Y/S}$를 나타낸다(그 증명의 첫
부분을 보라). 이제 $\zeta$는 $\beta'$가 유도하는 준동형
$\mathcal{K}/\mathcal{K}^2 \to \mathcal{G}$의 동치류에 대응한다.
주 \ref{defos-remark-extensions-of-relative-ringed-spaces}를 보라. 마찬가지로
...
(\ref{defos-equation-to-solve-ringed-spaces})에서와 같이 택하자. 보조정리
\ref{defos-lemma-extensions-of-relative-ringed-spaces}에 의해
...
\begin{proof}
보조정리 \ref{defos-lemma-extensions-of-relative-ringed-spaces}를
...
$0 \to \mathcal{G} \to \mathcal{O}_{X'} \to \mathcal{O}_X \to 0$
보조정리
\ref{defos-lemma-extensions-of-relative-ringed-spaces-functorial}을
$X \to S \to S'$와 $c : \mathcal{J} \to \mathcal{G}$에 적용하면,
$c$ 및 $f : X \to S$와 양립하는 $S'$-사상 $X' \to S'$가 존재할
필요충분조건은 $\zeta$가 $\xi$로 가는 것임을 알 수 있다. 이는
정확히 $\mathcal{O}_{X'}$이
(\ref{defos-equation-to-solve-ringed-spaces})의 해라는 뜻이다.
\end{proof}

\begin{remark}
\label{defos-remark-parametrize-solutions-ringed-spaces}
보조정리 \ref{defos-lemma-parametrize-solutions-ringed-spaces}의 상황에서는
다음 복합체의 사상들이 있다.
$$
...
$\xi$의 상이 (\ref{defos-equation-to-solve-ringed-spaces})의 해의 존재에
...
\noindent
이 절에서는 절 \ref{defos-section-deformations-ringed-spaces}의 결과들이
...
스킴의 두꺼워짐은 절 \ref{defos-section-thickenings-spaces}의 의미에서
...
\ref{defos-lemma-deform-module})에 의해 $X'$ 안의 $X$의 아이디얼층은
...
$$
여기서 세로 화살표들은 $f$-사상이다.
(\ref{defos-equation-to-solve-ringed-spaces})와 비교하라. 따라서 (1)은
보조정리 \ref{defos-lemma-choices-ringed-spaces}에서, (2)는 보조정리
\ref{defos-lemma-huge-diagram-ringed-spaces}의 (2)에서 따른다. 사상의 심화,
절 \ref{more-morphisms-section-netherlander}에서 스킴의 사상에 대해
정의한 $\NL_{X/S}$는 절 \ref{defos-section-deformations-ringed-spaces}에서
...
\noindent
이 절은 환 달린 토포스에 대한 절 \ref{defos-section-thickenings-spaces}의
...
상황 \ref{defos-situation-morphism-thickenings-ringed-topoi}에서 준동형
...
\ref{defos-section-thickenings-ringed-topoi}에서 도입한 표기를 자유롭게
...
0 \to \mathcal{L} \to \mathcal{G}' \to \mathcal{G} \to 0
$$
(\ref{defos-equation-extension-ringed-topoi})에서와 같은 준동형
$\varphi : \mathcal{F} \to \mathcal{G}$ 및
...
이 주어졌다고 하자. $c_{\mathcal{F}'} = c$인 완전열
(\ref{defos-equation-extension-ringed-topoi})이 하나라도 존재하면, 그러한
확대들의 동형류 집합은
...
$$
이 원소가 영이라는 것은 $c_{\mathcal{F}'} = c$인 완전열
(\ref{defos-equation-extension-ringed-topoi})이 존재하기 위한 필요충분조건이다.
\end{lemma}

\begin{proof}
먼저 $\mathcal{K}$가 단사 $\mathcal{O}$-가군이면
$c_{\mathcal{F}'} = c$인 완전열
(\ref{defos-equation-extension-ringed-topoi})이 실제로 존재함을 보이자. 평탄
...
0 \to \mathcal{K}' \to \mathcal{E}' \to \mathcal{F} \to 0
$$
이는 (\ref{defos-equation-extension-ringed-topoi})에서와 같고
...
존재하면 보조정리 \ref{defos-lemma-inf-ext-ringed-topoi}와 단사 가군
...
\mathcal{F}'_{c, triv} = \mathcal{F} \oplus \mathcal{K}
$$
$\pi$에 딸린 분할 (\ref{defos-equation-splitting-ringed-topoi})과 준동형 $c$를 사용하여
$\mathcal{O}'$-가군 구조를 정의하면 확대
(\ref{defos-equation-extension-ringed-topoi})을 얻는다.
$\mathcal{F}'_{c, triv}$를 $c$와 자명화 $\pi$에 대응하는,
$\mathcal{F}$를 $\mathcal{K}$로 확대한 {\it 자명한 확대}라고 부른다.
(\ref{defos-equation-extension-ringed-topoi})과 같은 임의의 확대
...
\ref{defos-lemma-inf-ext-ringed-topoi}에 의해
$\mathcal{F}' \mapsto \xi_{\mathcal{F}'}$
는 전단사 대응
$$
\left\{
\begin{matrix}
\text{확대의 동형류}\\
\mathcal{F}'\text{는 (\ref{defos-equation-extension-ringed-topoi})과 같고 }
c = c_{\mathcal{F}'}
\end{matrix}
\right\}
\longrightarrow
...
\left\{
\begin{matrix}
\mathcal{F}'_2\text{는 (\ref{defos-equation-extension-ringed-topoi})과 같고 }\\
c_2 = c_{\mathcal{F}'_2}\text{이며 }\mathcal{K} = \mathcal{K}_2
\end{matrix}
\right\}
\longrightarrow
\left\{
\begin{matrix}
\mathcal{F}'_1\text{는 (\ref{defos-equation-extension-ringed-topoi})과 같고 }\\
c_1 = c_{\mathcal{F}'_1}\text{이며 }\mathcal{K} = \mathcal{K}_1
\end{matrix}
\right\}
$$
...
\ref{defos-remark-extension-functorial-ringed-topoi}에서와 같다고 하자.
...
$$
이 상황에서 주 \ref{defos-remark-trivial-extension-ringed-topoi}의 구성은
다음 가환 도표를 유도한다.
$$
\xymatrix{
\{\mathcal{F}'_2\text{는 (\ref{defos-equation-extension-ringed-topoi})과 같고 }
...
여기서 오른쪽 세로 준동형은 $\Ext$의 함자성과 준동형
$\mathcal{K}_2 \to \mathcal{K}_1$로 주어지고, 왼쪽 세로 준동형은
주 \ref{defos-remark-extension-functorial-ringed-topoi}에서 구성한 것이다.
\end{remark}
...
$\mathcal{I}_i$, $h : (\Sh(\mathcal{D}_1), \mathcal{O}'_1) \to
(\Sh(\mathcal{D}_2), \mathcal{O}'_2)$는 주
\ref{defos-remark-extension-functorial-ringed-topoi}에서와 같다고 하자. 특히
...
\ref{defos-lemma-inf-obs-ext-ringed-topoi}의 절차에 따르면
...
\ref{defos-remark-short-exact-sequence-thickenings-ringed-topoi}를 보라. 각
...
$$
이 주어졌다고 하자. 이는 $\mathcal{K} = \mathcal{K}_2$인
(\ref{defos-equation-extension-ringed-topoi})과 같고
$c_{\mathcal{F}'_2} = c_2$이다. 주
\ref{defos-remark-extension-functorial-ringed-topoi}의 함자성을 적용하여
$\mathcal{O}'_1$-가군의 확대 $\mathcal{F}'_1$을 얻을 수 있다. 이 특별한
경우의 $\mathcal{F}'_1$은 아래에서 설명한다. 주
\ref{defos-remark-trivial-extension-ringed-topoi}에 따라, 주
\ref{defos-remark-short-exact-sequence-thickenings-ringed-topoi}의 표준 자명화
$\pi : (\Sh(\mathcal{D}_1), \mathcal{O}'_1) \to
...
$$
보조정리 \ref{defos-lemma-inf-obs-ext-ringed-topoi}를 보라. 이 상황에서 짧은
완전열
...
\ref{defos-lemma-inf-obs-ext-ringed-topoi}의 증명은
...
(\ref{defos-equation-extension-ringed-topoi})과 같은 확대이다. 특히
...
\begin{proof}
이는 보조정리 \ref{defos-lemma-inf-map-ringed-topoi}의 특수한 경우이지만 직접
...
$(f, f')$를 상황 \ref{defos-situation-morphism-thickenings-ringed-topoi}에서와
...
사상이라고 가정하자(정의
\ref{defos-definition-strict-morphism-thickenings-ringed-topoi}). 그러면 다음
조건들이 동치이다.
\begin{enumerate}
...
가정해도 좋고 그렇게 한다. 보조정리
\ref{defos-lemma-deform-module-ringed-topoi}에 의해 다음 짧은 완전열이 있다.
$$
...
\begin{proof}
보조정리 \ref{defos-lemma-inf-map-special-ringed-topoi}와
\ref{defos-lemma-deform-module-ringed-topoi}을 결합하면 된다.
\end{proof}
...
\begin{proof}
보조정리 \ref{defos-lemma-inf-map-special-ringed-topoi}의 증명으로부터, 다음
...
\ref{defos-lemma-inf-obs-map-special-ringed-topoi}에서 따른다.
$$
\Ext^1_\mathcal{O}(Li^*\mathcal{F}', \mathcal{I}\mathcal{G}') =
\Ext^1_\mathcal{O}(\mathcal{F},
\mathcal{G} \otimes_\mathcal{O} f^*\mathcal{J})
$$
실제로 보조정리 \ref{defos-lemma-deform-module-ringed-topoi}에 의해
$\mathcal{I}\mathcal{G}' =
\mathcal{G} \otimes_\mathcal{O} f^*\mathcal{J}$
이다. 한편 다음에 유의하자.
$$
...
의 핵으로 계산됨을 알 수 있다. 이 핵은 보조정리
\ref{defos-lemma-deform-module-ringed-topoi}의 마지막 명제에 의해 영이다.
...
\begin{proof}[둘째 증명]
보조정리 \ref{defos-lemma-inf-obs-map-ringed-topoi}를 다음과 같이 적용할 수 있다.
보조정리 \ref{defos-lemma-deform-module-ringed-topoi}에 의해
...
\begin{proof}
그러한 가군이 하나 존재한다고 가정하면, 보조정리
\ref{defos-lemma-deform-module-ringed-topoi}에 의해 표준 준동형
$$
f^*\mathcal{J} \otimes_\mathcal{O} \mathcal{F} \to
\mathcal{I} \otimes_\mathcal{O} \mathcal{F}
$$
...
\ref{defos-lemma-inf-ext-ringed-topoi}를 적용하자. 보조정리
\ref{defos-lemma-deform-module-ringed-topoi}에 의해 대응하는 확대
...
\mathcal{I} \otimes_\mathcal{O} \mathcal{F}$
은 동형사상이다.
\item 보조정리 \ref{defos-lemma-inf-obs-ext-ringed-topoi}의 동치류
...
\begin{proof}
이는 보조정리 \ref{defos-lemma-deform-module-ringed-topoi}에 주어진
$\mathcal{O}_{\mathcal{B}'}$ 위에서 평탄한 $\mathcal{O}'$-가군의
특징짓기와 보조정리 \ref{defos-lemma-inf-obs-ext-ringed-topoi}에서 바로 따른다.
\end{proof}
...
$$
가로 화살표들은 상황
\ref{defos-situation-morphism-thickenings-ringed-topoi}에서와 같은 1차
두꺼워짐이다.
$\mathcal{I} = \Ker(i^\sharp) \subset \mathcal{O}'$ 및
...
\end{enumerate}
(1) $+$ (2)로부터 $\mathcal{I} = f^*\mathcal{J}$가 따름에 유의하자
(보조정리 \ref{defos-lemma-deform-module-ringed-topoi}를 $\mathcal{O}'$에
...
\ref{defos-lemma-inf-obs-ext-ringed-topoi}의 동치류
...
\begin{proof}
위에서 $\mathcal{I} = f^*\mathcal{J}$임을 보았으므로 (1)은 보조정리
\ref{defos-lemma-inf-obs-ext-rel-ringed-topoi}에서 따른다. (2)는 보조정리
\ref{defos-lemma-inf-ext-rel-ringed-topoi}에서, (3)은 보조정리
\ref{defos-lemma-inf-map-rel-ringed-topoi}에서 따른다.
\end{proof}
...
상황 \ref{defos-situation-morphism-flat-thickenings-ringed-topoi}에서 장애 동치류
...
\begin{proof}
주 \ref{defos-remark-obstruction-extension-functorial-ringed-topoi}에서 따른다.
\end{proof}
...
(주 \ref{defos-remark-short-exact-sequence-thickenings-ringed-topoi}).
\end{situation}

\begin{lemma}
\label{defos-lemma-verify-iv-ringed-topoi}
상황 \ref{defos-situation-ses-flat-thickenings-ringed-topoi}에서 가군
...
\ref{defos-lemma-flat-ringed-topoi})는
$\Ext^1_\mathcal{O}(\mathcal{F},
...
\ref{defos-lemma-flat-ringed-topoi})와 같다.
\end{lemma}
...
보조정리 \ref{defos-lemma-deform-module-ringed-topoi}에 의해 두 가군은
...
$$
이는 상황 \ref{defos-situation-ses-flat-thickenings-ringed-topoi}의 가정들과
$\mathcal{F}$가 $\mathcal{O}_\mathcal{B}$ 위에서 평탄하다는 사실에서
따른다. 장애 동치류에 대한 명제는 주
\ref{defos-remark-complex-thickenings-and-ses-modules-ringed-topoi}의 결과를 이
특수한 상황에 그대로 옮긴 것이다.
\end{proof}
...
과 (\ref{defos-equation-morphism-thickenings-ringed-topoi})에서와 같은
두꺼워짐의 사상 $(f, f')$를 찾는다. 여기서 $\Ker(i^\sharp)$를
$\mathcal{G}$와 동일시하며 $(f')^\sharp$가 주어진 사상 $c$를 유도해야
한다. 이때 $(\Sh(\mathcal{C}'), \mathcal{O}')$을
(\ref{defos-equation-to-solve-ringed-topoi})의 {\it 해}라고 부른다.
...
\end{equation}
이 도표의 앞면과 뒷면은 (\ref{defos-equation-to-solve-ringed-topoi})의 해이다.
...
이 원소가 영인 것은 $\nu$와 양립하며
(\ref{defos-equation-huge-1-ringed-topoi})에 들어맞는 환 달린 토포스의 사상
$(\Sh(\mathcal{C}'_2), \mathcal{O}'_2) \to
(\Sh(\mathcal{C}'_1), \mathcal{O}'_1)$이 존재하기 위한 필요충분조건이다.
\item $\nu$와 양립하며 (\ref{defos-equation-huge-1-ringed-topoi})에 들어맞는 사상
$(\Sh(\mathcal{C}'_2), \mathcal{O}'_2) \to
...
\begin{proof}
이 보조정리의 증명은 보조정리
\ref{defos-lemma-huge-diagram-ringed-spaces}의 증명과 같다. 독자에게 이 증명
...
\mathcal{G}_2)
$$
(\ref{defos-equation-huge-1-ringed-topoi})에 들어맞고 $\nu$와 양립하는
환 달린 토포스의 사상
$(\Sh(\mathcal{C}'_2), \mathcal{O}'_2) \to
...
(\ref{defos-equation-huge-2-ringed-topoi})가 주어졌고(바탕 토포스에서
...
$b|_{\mathcal{I}'} : \mathcal{I}' \to \mathcal{G}_2$를 유도한다.
$a$와 $b$는 모두 $(\mathcal{I}')^2$을 영으로 보낸다. 더욱이
(\ref{defos-equation-huge-2-ringed-topoi})의 왼쪽 정사각형이 가환하므로
$\mathcal{G}_2$로 가는 사상으로서 $a$와 $b$는
...
따라서 어떤 준동형 $\Omega \to \mathcal{G}_2$에 대하여 $\xi$가 합성
$\mathcal{I}/\mathcal{I}^2 \to \Omega \to \mathcal{G}_2$일 때, 그리고
그때에만 $[\xi]$가 영이다. (\ref{defos-equation-huge-2-ringed-topoi})에
들어맞는 환층의 준동형
...
\mathcal{O}'_2
$$
은 $\mathcal{I}'$을 영으로 보내므로, (\ref{defos-equation-huge-2-ringed-topoi})에
...
(\ref{defos-equation-to-solve-ringed-topoi})의 해가 존재하면, 해들의 동형류
집합은\par\noindent
$\Ext^1_\mathcal{O}(
...
\begin{proof}
(\ref{defos-equation-to-solve-ringed-topoi})의 두 해 $\mathcal{O}'_1$과
$\mathcal{O}'_2$가 주어지면, 보조정리
\ref{defos-lemma-huge-diagram-ringed-topoi}에 의해 준동형
...
\medskip\noindent
동치류 $\xi$에 대하여 보조정리
\ref{defos-lemma-NL-represent-ext-class-ringed-topoi}에서와 같이
...
\begin{proof}
이를 증명하기 위해, (\ref{defos-equation-to-solve-ringed-topoi})이 다음
도표로 주어지는 경우에 앞의 결과들을 적용하자.
$$
\xymatrix{
0 \ar[r] &
...
보조정리 \ref{defos-lemma-choices-ringed-topoi}에서 따른다. 전단사의 직접
구성은 아래의 주를 보라.
\end{proof}
...
\ref{defos-lemma-extensions-of-relative-ringed-topoi}에서와 같이 택하자.
보조정리에서와 같은 확대
$0 \to \mathcal{G} \to \mathcal{O}' \to \mathcal{O} \to 0$
를 생각하자. 집합층 $\mathcal{E}$와 다음 가환 도표를 택할 수 있다.
$$
...
\end{enumerate}
보조정리 \ref{defos-lemma-extensions-of-relative-ringed-topoi}를 보라. 다음
환 달린 토포스의 사상
$$
g' :
...
보조정리 \ref{defos-lemma-huge-diagram-ringed-topoi}을 다음 도표에 적용하면
보조정리의 명제를 얻을 수 있다.
$$
\xymatrix{
& 0 \ar[r] &
...
$$
여기에 보조정리 \ref{defos-lemma-extensions-of-relative-ringed-topoi}와
\ref{defos-lemma-huge-diagram-ringed-topoi}의 증명에 나오는 구성들 사이의
양립성을 함께 사용한다. 그 양립성의 명제와 증명은 생략한다. 직접
논증은 아래의 주를 보라.
\end{proof}
...
를 보조정리 \ref{defos-lemma-extensions-of-relative-ringed-topoi-functorial}에서와
같이 택하자. $c : g^{-1}\mathcal{F} \to \mathcal{G}$에 따른 밀어내기를
사용하여 다음 확대를 구성할 수 있다.
$$
\xymatrix{
...
\ref{defos-lemma-extensions-of-relative-ringed-topoi}에 의해 이 확대들은
다음 등식의 왼쪽 항으로 분류된다.
$$
...
정의에서 따른다. 따라서 보조정리
\ref{defos-lemma-extensions-of-relative-ringed-topoi-functorial}을 보이려면
$\mathcal{O}'_1$이 $\xi$의 상에 대응하고 $\mathcal{O}'_2$가
$\zeta$의 상에 대응함을 보이면 충분하다. $\xi$와
$\mathcal{O}'_1$ 사이의 대응은 주
\ref{defos-remark-extensions-of-relative-ringed-topoi}에서 동치류 $\xi$를
구성한 방법에서 바로 알 수 있다. $\zeta$와 $\mathcal{O}'_2$ 사이의
대응을 위해 먼저 다음 가환 도표를 택한다.
$$
\xymatrix{
...
를 나타낸다. 이 사상은 보조정리
\ref{defos-lemma-extensions-of-relative-ringed-topoi-functorial}의 명제에서
사용한 것이다(그 증명의 첫 부분을 보라). 이제 $\zeta$는 $\beta'$가
유도하는 준동형 $\mathcal{K}/\mathcal{K}^2 \to \mathcal{G}$의
동치류에 대응한다. 주
\ref{defos-remark-extensions-of-relative-ringed-topoi}를 보라. 마찬가지로
확대 $\mathcal{O}'_2$는 $\alpha'$가 유도하는 준동형
$\mathcal{J}/\mathcal{J}^2 \to \mathcal{G}$에 대응한다. 위의 가환
도표에 의해 이 준동형은 $\beta'$가 유도하는 준동형
$\mathcal{K}/\mathcal{K}^2 \to \mathcal{G}$와 다음 준동형의 합성이다.
...
(\ref{defos-equation-to-solve-ringed-topoi})에서와 같이 택하자. 보조정리
\ref{defos-lemma-extensions-of-relative-ringed-topoi}에 의해
...
\begin{proof}
보조정리 \ref{defos-lemma-extensions-of-relative-ringed-topoi}를
...
$0 \to \mathcal{G} \to \mathcal{O}' \to \mathcal{O} \to 0$
보조정리
\ref{defos-lemma-extensions-of-relative-ringed-topoi-functorial}을 다음에
적용하자.
$$
...
이는 정확히 $\mathcal{O}'$이
(\ref{defos-equation-to-solve-ringed-topoi})의 해라는 뜻이다.
\end{proof}
...
\noindent
이 절에서는 절 \ref{defos-section-deformations-ringed-topoi}의 결과들이
...
절 \ref{defos-section-thickenings-ringed-topoi}의 의미에서 두꺼워짐이다.
\end{enumerate}
\end{lemma}
...
경우에는 절 \ref{defos-section-thickenings-ringed-topoi}의 조건들로부터
...
두꺼워짐들의 사상이다. 보조정리 \ref{defos-lemma-match-thickenings}에 의해
이는 환 달린 토포스의 두꺼워짐들의 사상으로 옮겨진다. 공간의 사상의
심화, 보조정리
\ref{spaces-more-morphisms-lemma-deform}
(또는 더 일반적인 보조정리 \ref{defos-lemma-deform-module-ringed-topoi})에
...
$$
(\ref{defos-equation-to-solve-ringed-topoi})와 비교하라. (2)에서와 같은
...
(1)은 보조정리 \ref{defos-lemma-choices-ringed-topoi}에서, (2)는 보조정리
\ref{defos-lemma-huge-diagram-ringed-topoi}의 (2)에서 따른다. 공간의 사상의
심화, 절
\ref{spaces-more-morphisms-section-netherlander}
에서 대수공간의 사상에 대하여 정의한 $\NL_{X/Y}$는
절 \ref{defos-section-deformations-ringed-topoi}에서 사용한 $\NL_{X/Y}$와
일치함에 유의하라.
\end{proof}
...
$$
보조정리 \ref{defos-lemma-huge-diagram-ringed-topoi}에 의해 이는 함자가
충실충만임을 뜻한다. 한편 두 범주에 각각 해
$\mathcal{O}_X \oplus \mathcal{F}$와
$\mathcal{O}_X \oplus \mathcal{G}$가 있으므로, 보조정리
\ref{defos-lemma-choices-ringed-topoi}에 의해 이 함자는 본질적으로
전사적이다.
\end{proof}
...
$$
FT :
\{(\ref{defos-equation-to-solve-ringed-topoi})\text{의 }
...
\begin{proof}
(\ref{defos-equation-to-solve-ringed-topoi})의 $\mathcal{F}$에 대한 해는 특히
...
보조정리 \ref{defos-lemma-thickening-space-quasi-coherent}의 동치
...
\medskip\noindent
이 절의 내용은 절 \ref{defos-section-thickenings-ringed-topoi}에서 정의한 환
...
$\mathcal{G}^n = 0$으로 놓고 보조정리 \ref{defos-lemma-lift-complex}를
적용한다. 보조정리가 만드는 준동형을
...
에 보조정리 \ref{defos-lemma-lift-complex}를 적용한다(사용한 글자의 순서가
...
\begin{enumerate}
\item 보조정리 \ref{defos-lemma-canonical-class}에서 구성한 동치류
$\omega(K_0) \in
...
것과 보조정리 \ref{defos-lemma-induced-map}에 의해 $\beta$에 대해서도 같은
것이 참이다. 이제 다음 도표를 보자.
$$
\xymatrix@C=1.2em{
...
\ref{defos-lemma-induced-map}에 의해 $H^{n - 1}(\beta)$가 전사 준동형임을
...
보조정리 \ref{defos-lemma-inf-obs-map-defo-complex}에 의해 올림
...
는 복합체의 사상이다. 보조정리 \ref{defos-lemma-lift-map-complexes}에 의해
...
\mathcal{H}^\bullet \to \mathcal{G}_0^\bullet \to 0
$$
이 존재한다. 보조정리 \ref{defos-lemma-canonical-class-obstruction}의 증명과
똑같이, 이 열이 동형사상
$\alpha_0 :
