% Generated mechanically from frozen input canon/ko-kr/integration-20260908-r10-p03-register/inputs/p11/ko/cotangent.tex.
...
주 \ref{cotangent-remark-resolution}을 보라. 이는 {\v C}ech 복합체로 코호몰로지를
...
절 \ref{cotangent-section-compute-L-pi-shriek}에서 연구하는 범주
...
$$
보조정리 \ref{cotangent-lemma-compute-cotangent-complex}를 보라. 우리는 환 준동형의
코탄젠트 복합체의 기본 성질들을 증명할 때 일관되게 이 관점을 사용하려고
했다. 특히 표준 분해의 존재에 의존하지 않고도 모든 결과를 증명할 수
있지만, 여기서는 그렇게 하지 않았다. 이 이론은 상당히 만족스럽다.
다만 기본 삼각형(명제 \ref{cotangent-proposition-triangle})의 증명에는 유도 하방
느낌표 함자에 관한 이론이 조금 더 필요할 수도 있다. 다른 방법도 제시하기
위하여 주 \ref{cotangent-remark-triangle}과 \ref{cotangent-remark-explicit-map}에서 표준
...
$A$ 위의 $B$의 표준 분해라 하고, $\pi$를 (\ref{cotangent-equation-pi})와 같이
두자. 그러면
$$
L\pi_!(\mathcal{F}) = \mathcal{F}(P_\bullet, \epsilon)
$$
...
\begin{proof}[첫 번째 증명]
보조정리 \ref{cotangent-lemma-identify-pi-shriek}를 적용하겠다. 각 항 $P_n$이
...
때문에 이렇게 쓸 수 있다). 주 \ref{cotangent-remark-variant-cotangent-complex}의
표기를 사용하면
$$
...
(\ref{cotangent-equation-pi})와 같이 두자. $D(B)$에서 다음 표준 동형사상이 있다.
$$
...
$\underline{B}$-가군층이다. 보조정리 \ref{cotangent-lemma-pi-shriek-standard}에
의해 우변은 이 층을 표준 분해에서 취한 값으로 계산된다. 이것이 바로
좌변의 정의이다(정의 \ref{cotangent-definition-cotangent-complex-ring-map}).
\end{proof}

\begin{lemma}
\label{cotangent-lemma-pi-lower-shriek-constant-sheaf}
환 준동형 $A\to B$가 주어지고 $\pi$를 (\ref{cotangent-equation-pi})와 같이 두면
$L\pi_!(\pi^{-1}M)=M$이다.
\end{lemma}

\begin{proof}
보조정리 \ref{cotangent-lemma-identify-pi-shriek}에 의해
...
\begin{proof}
직접 계산하여 증명하겠다. 보조정리
\ref{cotangent-lemma-compute-cotangent-complex}의 동일시를 사용한다.
...
$B$가 환 $A$ 위의 다항식 대수이면, $\pi$를 (\ref{cotangent-equation-pi})와 같이
둘 때 $\pi_!$는 완전하고 $\pi_!\mathcal{F}=\mathcal{F}(B\to B)$이다.
\end{lemma}

\begin{proof}
보조정리 \ref{cotangent-lemma-identify-pi-shriek}에 의해 $B$ 위의 상수 단체 대수를
...
\begin{proof}
보조정리 \ref{cotangent-lemma-compute-cotangent-complex}과
\ref{cotangent-lemma-pi-lower-shriek-polynomial-algebra}에서 바로 따른다.
\end{proof}
...
\end{enumerate}
보조정리 \ref{cotangent-lemma-polynomial}에 의해 $A$ 위의 유한 생성 다항식 대수
...
$\underline{B}$를 (\ref{cotangent-equation-pi})와 같이 두자. 모든
...
\begin{proof}
보조정리 \ref{cotangent-lemma-identify-pi-shriek}와 명제
\ref{cotangent-proposition-polynomial}에 의해, 유한형 다항식 대수에서의
...
\begin{proof}
보조정리 \ref{cotangent-lemma-compute-cotangent-complex}과
\ref{cotangent-lemma-pi-shriek-finite}를 적용한다.
\end{proof}
...
\ref{cotangent-lemma-identify-pi-shriek}를 보라. 명제 \ref{cotangent-proposition-polynomial}
증명의 형식적인 부분은 분해가 존재함을 보인다. 또한 보조정리
\ref{cotangent-lemma-pi-shriek-standard}의 첫 번째 증명에서 $A$ 위의 $B$의 표준
분해가 분해임을 보았다(따라서 이 용어법은 충돌을 일으키지 않는다).
그러나 명제 \ref{cotangent-proposition-polynomial}의 증명에 나오는 논증은 단체적
방법, 절 \ref{simplicial-section-standard}의 단체 계산을 사용하지 않고도
분해의 존재를 보인다. 더 나아가 분해를 {\it 어떻게} 택하든 보조정리
\ref{cotangent-lemma-compute-cotangent-complex}에 의해 $D(B)$에서 표준 동형사상
$$
...
$\underline{B}$를 (\ref{cotangent-equation-pi})와 같이 두자. 임의의
...
\ref{cotangent-remark-resolution}의 결과들을 사용하겠다. $A$ 위의 $B$의 분해
...
환 준동형 $A\to B$가 주어졌다고 하고 $\pi$, $\mathcal{O}$,
$\underline{B}$를 (\ref{cotangent-equation-pi})와 같이 두자. $D(\textit{Ab})$에서
$$
...
\begin{proof}
보조정리 \ref{cotangent-lemma-O-homology-B-homology}를 적용하면 된다(오른쪽의 첫
등식은 보조정리 \ref{cotangent-lemma-compute-cotangent-complex}이다).
\end{proof}
...
\begin{proof}
더 언급하지 않고 주 \ref{cotangent-remark-resolution}의 관찰들을 사용하겠다.
...
\ref{cotangent-lemma-cotangent-complex-polynomial-algebra}) 결론을 얻는다.
\end{proof}
...
\ref{cotangent-proposition-polynomial} 증명의 절차를 따른다. 주
\ref{cotangent-remark-resolution}의 논의도 보라.
...
자명한 Kan 올뭉치이다. 명제 \ref{cotangent-proposition-polynomial} 증명의 절차에서
...
\ref{cotangent-proposition-polynomial} 증명의 절차에 따라 두 사상
...
\ref{cotangent-section-compute-L-pi-shriek}에서 생각한 범주들이라 하자. 다음 함자가
있다.
$$
...
$$
을 얻는다. 보조정리 \ref{cotangent-lemma-compute-cotangent-complex}에 의해 이는
$L_{B/A}\to L_{B'/A'}$를 준다. 이 사상이 위에서 단체 분해로 정의한
사상과 일치한다는 확인은 생략한다.

\begin{lemma}
\label{cotangent-lemma-flat-base-change}
(\ref{cotangent-equation-commutative-square})가 준동형사상
$B\otimes_A^\mathbf{L}A'=B'$을 유도한다고 가정하자.
(\ref{cotangent-equation-double-square})의 표기와
$\mathcal{F}'\in\textit{Ab}(\mathcal{C}')$에 대하여
$L\pi_!(g^{-1}\mathcal{F}')=L\pi'_!(\mathcal{F}')$이다.
\end{lemma}

\begin{proof}
더 언급하지 않고 주 \ref{cotangent-remark-resolution}의 결과들을 사용한다. 사이트
...
\begin{proof}
등식 (\ref{cotangent-equation-double-square})에서 도입한 표기를 사용하겠다. 다음이
성립한다.
$$
...
$$
첫 등식은 보조정리 \ref{cotangent-lemma-compute-cotangent-complex}에서, 둘째 등식은
사이트 코호몰로지, 보조정리
...
와 같다. 보조정리 \ref{cotangent-lemma-flat-base-change}와 $L_{B'/A'}$가
...
정사각형 (\ref{cotangent-equation-commutative-square})가 주어지고 다음 도표를
...
\ref{cotangent-lemma-special-case-triangle}에 의해 $D(B\times C)$에 완전 삼각형
$$
...
\ref{cotangent-lemma-flat-base-change-cotangent-complex})에 의해 사상
...
\end{equation}
이는 명제 \ref{cotangent-proposition-triangle}에서 증명한다. 다른 접근법은 주
\ref{cotangent-remark-triangle}을 보라.
...
\ref{cotangent-section-compute-L-pi-shriek}에서 생각한 범주들이라 하자. 다음
함자들이 있다.
$$
\begin{matrix}
...
\ref{cotangent-lemma-compute-cotangent-complex}의 코탄젠트 복합체 동일시와 다음
보조정리들로부터 원하는 완전 삼각형을 얻을 것이다.

\begin{lemma}
\label{cotangent-lemma-triangle-ses}
(\ref{cotangent-equation-three-maps})의 표기 아래에서 다음과 같이 두자.
$$
\begin{matrix}
...
(\ref{cotangent-equation-three-maps})의 표기 아래에서 $C$가 $B$ 위의 다항식
...
\ref{cotangent-lemma-pi-lower-shriek-polynomial-algebra}에 의해 이것은
...
(\ref{cotangent-equation-three-maps})의 표기 아래에서
...
\ref{cotangent-lemma-pi-lower-shriek-constant-sheaf}에서 원하는 결과가 따른다.
...
\ref{cotangent-lemma-polynomial-on-top}에 의해 $n>0$이면
...
\begin{proof}
보조정리 \ref{cotangent-lemma-triangle-ses}의 층의 짧은 완전열에 유도 함자
...
$$
을 얻는다. 보조정리 \ref{cotangent-lemma-triangle-compute-lower-shriek}와
\ref{cotangent-lemma-compute-cotangent-complex}을 사용하면 둘째 항과 셋째 항이
...
\ref{cotangent-lemma-compute-cotangent-complex}에서 따른다.
\end{proof}
...
\ref{cotangent-proposition-triangle} 증명의 핵심은 $\overline{Q}_\bullet$이 $B$
...
의해 명제 \ref{cotangent-proposition-triangle}의 완전 삼각형은 다음 단체
...
\ref{cotangent-remark-resolution}과 핵심 사실에 의해 우변의 복합체는 $D(C)$에서
$L_{C/B}$를 나타낸다.
...
$L_{B\times C/A}\to L_{B/A}\oplus L_{C/A}$는 $D(B\times C)$에서
준동형사상이므로(보조정리 \ref{cotangent-lemma-cotangent-complex-product}), 평탄
...
\ref{cotangent-remark-triangle}의 표기 $P_\bullet$, $Q_\bullet$,
...
다항식 대수이고, 주 \ref{cotangent-remark-triangle}에서와 같이 사이트 코호몰로지,
...
$$
은 준동형사상이고(주 \ref{cotangent-remark-resolution}), 하나와 다른 하나의 역을
합성하면 원하는 동일시를 얻는다.
\end{remark}
...
\ref{cotangent-lemma-flat-base-change}를 보라.
...
\begin{proof}
위의 논의(즉 보조정리 \ref{cotangent-lemma-flat-base-change})와 보조정리
\ref{cotangent-lemma-compute-cotangent-complex}에 따르면,
...
\begin{proof}
보조정리 \ref{cotangent-lemma-localize-at-bottom}과
\ref{cotangent-lemma-cotangent-complex-polynomial-algebra}에 의해
...
\begin{proof}
보조정리 \ref{cotangent-lemma-flat-base-change-cotangent-complex}에 의해
...
$$
과 $L_{B/B}$의 소멸(보조정리
\ref{cotangent-lemma-cotangent-complex-polynomial-algebra}) 및 가정한
...
\ref{cotangent-lemma-derived-diagonal}을 적용하여 $L_{B/B\otimes_AB}=0$을 얻고,
보조정리 \ref{cotangent-lemma-bootstrap}을 사용하여 결론을 얻는다. (2)--(5)는 각각
(1)의 특수한 경우이다. (6)은 보조정리
\ref{cotangent-lemma-colimit-cotangent-complex}에서 따른다. (7)과 (8)은 (엄밀)
...
\begin{proof}
완전 삼각형 (\ref{cotangent-equation-triangle})에서 바로 따른다.
\end{proof}
...
\begin{proof}
보조정리 \ref{cotangent-lemma-localize-on-top}에 의해
$L_{T^{-1}B/A}=L_{B/A}\otimes_BT^{-1}B$이고, 보조정리
\ref{cotangent-lemma-localize-at-bottom}에 의해
...
\begin{proof}
보조정리 \ref{cotangent-lemma-when-zero}에 의해 복합체 $L_{A^h/A}$,
$L_{A^{sh}/A}$, $L_{B^h/B}$, $L_{B^{sh}/B}$는 모두 영이다.
$A\to B\to B^h$에 기본 완전 삼각형 (\ref{cotangent-equation-triangle})을 사용하면
...
\ref{cotangent-lemma-when-zero}에서 ``에탈 환 준동형''에 대하여 한 일을
...
\begin{proof}
보조정리 \ref{cotangent-lemma-identify-H0}에 의해 코호몰로지 차수 $0$에서는
일치한다. 따라서 다른 코호몰로지 군들이 영임을 보이면 충분하다. 보조정리
\ref{cotangent-lemma-localize-on-top}에 의해 $g\in B$에 대하여
...
\ref{cotangent-lemma-when-zero}와 \ref{cotangent-lemma-localize-on-top}에 의해
$L_{B/A}=L_{A[x_1,\ldots,x_n]/A}\otimes B$이고, 보조정리
\ref{cotangent-lemma-cotangent-complex-polynomial-algebra}에서 결론이 따른다.
\end{proof}
...
\ref{cotangent-section-functoriality}에서 논의한 다음 도표가 유도하는 사상
...
\ref{cotangent-section-functoriality}의 사상 $L_{B/A}\to L_{B/A}$은 영과
호모토픽이다.
\end{lemma}

\begin{proof}
$P_\bullet$을 $A$ 위의 $B$의 표준 분해라 하자. 보조정리
\ref{cotangent-lemma-frobenius-homotopy}에 의해 $F_A$와 $F_B$가 유도하는 사상
...
동형사상을 유도한다. 한편 보조정리
\ref{cotangent-lemma-frobenius-acts-as-zero}에 의해 $L_{B/A}$에서 영을 유도한다.
\end{proof}
...
\ref{cotangent-section-compute-L-pi-shriek}에서처럼 $\mathcal{C}_{B/A}$ 위에서
작업하고, $\mathcal{J} \subset \mathcal{O}$를
$\mathcal{O} \to \underline{B}$의 핵이라 하자. 보조정리
\ref{cotangent-lemma-apply-O-B-comparison}에 의해 $L\pi_!(\mathcal{J}) = 0$임에
유의하자. 다음과 같이 두자.
$\Omega =  \Omega_{\mathcal{O}/A} \otimes_\mathcal{O} \underline{B}$
그러면 보조정리 \ref{cotangent-lemma-compute-cotangent-complex}에 의해
...
이 과정을 계속하자. 보조정리
\ref{cotangent-lemma-O-homology-B-homology}에 의해
...
(\ref{cotangent-equation-comparison-map})에서 나온다.
\end{lemma}
...
긴 완전열과 주 \ref{cotangent-remark-make-map}에서 보인
$H_1(\mathcal{C}_{B/A}, \mathcal{J}/\mathcal{J}^2)$ 및
$H_0(\mathcal{C}_{B/A}, \mathcal{J}/\mathcal{J}^2)$
의 소멸을 이용하면, 보조정리
\ref{cotangent-lemma-pi-lower-shriek-constant-sheaf}에 의해 원하는 결론을 얻는다.
...
(\ref{cotangent-equation-comparison-map})에서 이미 표준 사상
$$
...
$$
을 생각하자. 보조정리
\ref{cotangent-lemma-cotangent-complex-polynomial-algebra}와 가환대수학, 보조정리
\ref{algebra-lemma-NL-polynomial-algebra}에 의해 $D(P)$에서
$L_{P/A} = \Omega_{P/A}[0] = \NL_{P/A}$
이고, 보조정리 \ref{cotangent-lemma-surjection}과 가환대수학, 보조정리
...
보조정리 \ref{cotangent-lemma-relation-with-naive-cotangent-complex}의 비교 사상을
...
보조정리 \ref{cotangent-lemma-relation-with-naive-cotangent-complex}의 사상과 같다는
확인은 생략한다.
\end{remark}

\begin{remark}
\label{cotangent-remark-surjection}
주 \ref{cotangent-remark-make-map}의 표기를 사용하자. 거기서 제시한 논증은 스펙트럼
열의 미분
$$
...
\ref{cotangent-lemma-relation-with-naive-cotangent-complex})에 의해
...
\ref{cotangent-lemma-identify-pi-shriek}에 의해 이들은 상수 단체 아벨 군의 호몰로지
...
보조정리 \ref{cotangent-lemma-vanishing-symmetric-powers}의 상황에서
...
\ref{cotangent-section-compute-L-pi-shriek}에서처럼 $\mathcal{C}_{B/A}$ 위의
$\underline{B}$-가군 층
$\Omega = \Omega_{\mathcal{O}/A} \otimes_\mathcal{O} \underline{B}$
을 생각하자. 그러면 $E_1$-쪽이
$$
...
\ref{cotangent-lemma-identify-H0}). 이는 $A \to B$가 전사이기 때문이다(가환대수학,
보조정리 \ref{algebra-lemma-trivial-differential-surjective}). 보조정리
\ref{cotangent-lemma-vanishing-symmetric-powers}에 의해
$$
...
$$
첫 번째 등식은 보조정리 \ref{cotangent-lemma-apply-O-B-comparison}, 두 번째 등식은
사이트 위의 코호몰로지, 보조정리
...
\ref{cotangent-lemma-O-homology-B-homology}를 보라. 다음 사상을 생각하자.
$$
...
\ref{cotangent-lemma-map-tors-zero}이다.
\end{proof}

\begin{remark}
\label{cotangent-remark-elucidate-ss}
정리 \ref{cotangent-theorem-quillen-spectral-sequence}의 상황에서
$I = \Ker(A \to B)$로 두자. 그러면
$H^{-1}(L_{B/A}) = H_1(\mathcal{C}_{B/A}, \Omega) = I/I^2$이다. 보조정리
\ref{cotangent-lemma-surjection}를 보라. 따라서 주
\ref{cotangent-remark-first-homology-symmetric-power}에 의해
...
하자(주 \ref{cotangent-remark-resolution}). $J_n = \Ker(P_n \to B)$로 두자. 다음에
유의하자.
$$
\text{Tor}_2^{P_n}(B, B) = 
...
$$
따라서 주 \ref{cotangent-remark-surjection}에 의해 $H_2(L_{B/A})$는 표준적으로
$$
\Coker(\text{Tor}_2^{P_1}(B, B) \to \text{Tor}_2^{P_0}(B, B))
$$
와 같다. 이를 더 명시적으로 나타내기 위하여 예
\ref{cotangent-example-resolution-length-two}에서와 같이 $P_2$, $P_1$, $P_0$을
택한다. 다음과 같다고 주장한다.
$$
\text{Tor}_2^{P_1}(B, B) =
...
$$
이 있다. 이와 같이 주 \ref{cotangent-remark-elucidate-ss}에서 논의한 Quillen
...
(\ref{cotangent-equation-lichtenbaum-schlessinger})에 한 항을 덧붙이고 비교를
$\tau_{\geq -3}$까지 확장하는 논문 \cite{Doncel}도 참고할 수 있다.

\begin{lemma}
\label{cotangent-lemma-compare-higher}
위 상황에서 복합체 (\ref{cotangent-equation-lichtenbaum-schlessinger})를 $L$로
...
\ref{cotangent-remark-resolution}). $L_{B/A}$를
...
\ref{cotangent-lemma-polynomial-ring-unique}에 의해 $s$를 좌표가 $p_{s, t}$인
...
\ref{cotangent-remark-explicit-comparison-map}와 보조정리
\ref{cotangent-lemma-relation-with-naive-cotangent-complex}에 의해 이 사상은 동형사상
...
\ref{cotangent-example-resolution-length-two}에서처럼 $P_1$과 $P_2$를 택하자. 주
\ref{cotangent-remark-elucidate-degree-two}에서 완전열
$$
\wedge^2_B(J_0/J_0^2) \to \text{Tor}_2^{P_0}(B, B) \to H^{-2}(L_{B/A}) \to 0
$$
...
확인해야 한다. 예 \ref{cotangent-example-resolution-length-two}와 주
\ref{cotangent-remark-elucidate-degree-two} 및 \ref{cotangent-remark-surjection}에서 논의한
...
$$
이다. 이는 예 \ref{cotangent-example-resolution-length-two}에서 변수 $v$를 택한
...
$$
를 얻는다. 또한 보조정리 \ref{cotangent-lemma-compare-higher}의 증명에서 한 선택들을
...
명시적으로 구성하여 계산할 수 있다. 여기서는 (\ref{cotangent-equation-triangle})을
사용하겠다. 즉, 완전 삼각형
$$
...
$$
을 생각하자. 보조정리 \ref{cotangent-lemma-cotangent-complex-polynomial-algebra}에 의해
복합체 $L_{\mathbf{Z}[x_1, \ldots, x_n]/\mathbf{Z}}$는
$\Omega_{\mathbf{Z}[x_1, \ldots, x_n]/\mathbf{Z}}$와 준동형이다. 보조정리
\ref{cotangent-lemma-when-zero}에 의해 복합체 $L_{\mathbf{Z}/\mathbf{Z}}$는
$D(\mathbf{Z})$에서 영이다. 따라서 $L_{B/A}$는 영이 아닌 코호몰로지
군을 하나만 가지며, 보조정리 \ref{cotangent-lemma-surjection}에 의해 그것은 이
보조정리에 기술한 것과 같다.
\end{proof}
...
를 뜻한다. 따라서 보조정리
\ref{cotangent-lemma-flat-base-change-cotangent-complex}와
\ref{cotangent-lemma-special-case}에 의해
...
보조정리 \ref{cotangent-lemma-flat-base-change-cotangent-complex}에서 결론이 따른다.
\end{proof}
...
충분하다. 이는 (\ref{cotangent-equation-triangle})의 삼각형
$$
L_{P/A} \otimes_P^\mathbf{L} B \to L_{B/A} \to L_{B/P} \to
L_{P/A} \otimes_P^\mathbf{L} B[1]
$$
과 보조정리 \ref{cotangent-lemma-mod-Koszul-regular-ideal}에서 따른다. 실제로
$i \in \{-1, 0\}$가 아니면 $H^i(L_{B/P})$는 영이다. (왼쪽 항에는
보조정리 \ref{cotangent-lemma-cotangent-complex-polynomial-algebra}도 사용한다.)
\end{proof}
...
삼각형 (\ref{cotangent-equation-triangle})이다. 가운데 열은 환 준동형
$R \to B \to A \otimes_R B$에 대한 기본 삼각형
(\ref{cotangent-equation-triangle})이다. 다음으로 $E$는 오른쪽 위 모서리에
...
$A$와 $B$가 Tor-독립인 $R$-대수이면 (\ref{cotangent-equation-tensor-product})의
대상 $E$는 영이다. 이 경우
$$
L_{A \otimes_R B/R} =
L_{A/R} \otimes_A^\mathbf{L} (A \otimes_R B) \oplus
...
\begin{proof}
첫 두 명제는 보조정리 \ref{cotangent-lemma-flat-base-change-cotangent-complex}에서
...
(\ref{cotangent-equation-tensor-product})의 대상 $E$는 다음을 만족한다.
$$
H^i(E) =
\left\{
\begin{matrix}
...
사상뿔로 기술한 것을 사용한다. 보조정리 \ref{cotangent-lemma-compare-higher}에 의해
표준 절단 $\tau_{\geq -2}L_{B/R}$과
$\tau_{\geq -2}L_{A \otimes_R B/A}$는 Lichtenbaum--Schlessinger 복합체
(\ref{cotangent-equation-lichtenbaum-schlessinger})로 계산된다. 이 동형사상들은
함자성과 양립한다(주
\ref{cotangent-remark-functoriality-lichtenbaum-schlessinger}). 따라서 이
...
$B'$를 (\ref{cotangent-equation-to-solve})의 {\it 해}라고 부른다.
...
소멸은 (\ref{cotangent-equation-to-solve})의 해가 존재하기 위한 필요충분조건이다.
\item 해가 존재하면 해의 동형류들의 집합은
$\Ext^1_B(L_{B/A}, N)$ 아래의 주동차 공간이다.
\item 해 $B'$가 주어지면 (\ref{cotangent-equation-to-solve})에 들어맞는 $B'$의
...
\ref{cotangent-lemma-relation-with-naive-cotangent-complex}) 및
$H^0(L_{B/A}) = \Omega_{B/A}$(보조정리 \ref{cotangent-lemma-identify-H0})를 통하여
...
(\ref{cotangent-equation-triangle})은 긴 완전열
$$
\ldots \to
\Ext^1_B(L_{B/A'}, N) \to
...
\ref{cotangent-section-compute-L-pi-shriek}). 주부분의 확대
$$
...
$$
을 얻는다. 보조정리 \ref{cotangent-lemma-pi-shriek-standard}와 $L_{B/A}$의 항들의
평탄성에 의해
$L\pi_!(\Omega_{\mathcal{O}/\underline{A}} \otimes_\mathcal{O} \underline{M})
= L_{B/A} \otimes_B M = L_{B/A} \otimes_B^\mathbf{L} M$
이다. 보조정리 \ref{cotangent-lemma-pi-lower-shriek-constant-sheaf}에 의해
...
$A \to B$를 환 준동형이라 하고 $M$을 $B$-가군이라 하자.
(\ref{cotangent-equation-atiyah})의 사상
...
\noindent
이 구성들은 절 \ref{cotangent-section-functoriality}에서 논의한 것과 비슷한 함자성을
...
\ref{cotangent-lemma-pullback-cotangent-morphism-topoi}에 의해 $\mathcal{C}'$에 대하여,
...
보조정리 \ref{cotangent-lemma-compute-L-morphism-sheaves-rings}의 증명에서 임의의
...
\begin{proof}
보조정리 \ref{cotangent-lemma-compute-L-morphism-sheaves-rings} 및
\ref{cotangent-lemma-identify-H0}, 그리고 사이트 위의 가군, 보조정리
...
\begin{proof}
보조정리 \ref{cotangent-lemma-compute-L-morphism-sheaves-rings}에 의해 환 준동형에
대하여 이를 증명하면 충분하다. 환의 경우에는 보조정리
\ref{cotangent-lemma-cotangent-complex-product}가 바로 이 명제이다.
\end{proof}
...
\begin{proof}
주 \ref{cotangent-remark-triangle}과 \ref{cotangent-remark-explicit-map}에 기술한 방법으로 이
...
\ref{cotangent-remark-triangle}에 제시한 논증과 보조정리
\ref{cotangent-lemma-compute-L-product-sheaves-rings}에 의해 가능하다.
\end{proof}
...
\begin{proof}
이는 보조정리 \ref{cotangent-lemma-pullback-cotangent-morphism-topoi}의 특수한
경우이다.
\end{proof}
...
$$
이다. 이는 주 \ref{cotangent-remark-explicit-comparison-map}의 사상이며 $H^0$과
$H^{-1}$ 위에서 동형사상을 유도한다. 보조정리
\ref{cotangent-lemma-pullback-cotangent-morphism-topoi}에 의해
...
$\mathcal{B}$의 표준 분해라 하자(절 \ref{cotangent-section-cotangent-complex}). 모든
...
(\ref{cotangent-equation-atiyah-extension})은 단체 $\mathcal{P}_\bullet$-가군들의
...
$\mathcal{C}$를 사이트라 하고 $\mathcal{A} \to \mathcal{B}$를 환의 층들의
준동형이라 하자. $\mathcal{F}$를 $\mathcal{B}$-가군의 층이라 하자.
(\ref{cotangent-equation-atiyah-general})의 사상 $\mathcal{F} \to
L_{\mathcal{B}/\mathcal{A}} \otimes_\mathcal{B}^\mathbf{L} \mathcal{F}[1]$
...
\ref{cotangent-definition-cotangent-complex-morphism-sheaves-rings})를 생각한다. 사이트,
...
\begin{proof}
보조정리 \ref{cotangent-lemma-H0-L-morphism-sheaves-rings}의 특수한 경우이다.
\end{proof}
...
완전 삼각형은 보조정리 \ref{cotangent-lemma-triangle-sheaves-rings}의 완전 삼각형에서
...
\begin{proof}
보조정리 \ref{cotangent-lemma-compare-cotangent-complex-with-naive}의 특수한 경우이다.
\end{proof}
...
한다. 이러한 $X'$를 (\ref{cotangent-equation-to-solve-ringed-spaces})의
{\it 해}라고 부른다.
...
가 있으며, 이 원소의 소멸은 (\ref{cotangent-equation-to-solve-ringed-spaces})의 해가
존재하기 위한 필요충분조건이다.
\item 해가 존재하면 해의 동형류들의 집합은
$\Ext^1_{\mathcal{O}_X}(L_{X/S}, \mathcal{G})$ 아래의 주동차 공간이다.
\item 해 $X'$가 주어지면 (\ref{cotangent-equation-to-solve-ringed-spaces})에 들어맞는
$X'$의 자기동형사상들의 집합은
...
\ref{cotangent-lemma-compare-cotangent-complex-with-naive-ringed-spaces}) 및
$H^0(L_{X/S}) = \Omega_{X/S}$(보조정리
\ref{cotangent-lemma-H0-L-morphism-ringed-spaces})를 통하여 (2)와 (3)은 변형 이론,
...
\ref{cotangent-lemma-triangle-ringed-spaces}의 완전 삼각형은 긴 완전열
$$
\ldots \to
...
(\ref{cotangent-equation-to-solve-ringed-topoi})의 {\it 해}라고 부른다.
...
가 있으며, 이 원소의 소멸은 (\ref{cotangent-equation-to-solve-ringed-topoi})의 해가
존재하기 위한 필요충분조건이다.
\item 해가 존재하면 해의 동형류들의 집합은
$\Ext^1_\mathcal{O}(L_f, \mathcal{G})$ 아래의 주동차 공간이다.
\item 해 $X'$가 주어지면 (\ref{cotangent-equation-to-solve-ringed-topoi})에 들어맞는
...
\ref{cotangent-lemma-compare-cotangent-complex-with-naive-ringed-topoi}) 및
$H^0(L_f) = \Omega_f$(보조정리 \ref{cotangent-lemma-H0-L-morphism-ringed-topoi})를
...
\ref{cotangent-lemma-triangle-ringed-topoi}의 완전 삼각형은 긴 완전열
$$
\ldots \to
\Ext^1_\mathcal{O}(
...
(정의 \ref{cotangent-definition-cotangent-complex-morphism-ringed-spaces}).
\end{definition}

\noindent
특히 절 \ref{cotangent-section-cotangent-morphism-ringed-spaces}의 결과는
...
\begin{proof}
주의 \ref{cotangent-remark-map-sections-over-U}에 의해 복합체의 표준 사상
...
이는 보조정리 \ref{cotangent-lemma-stalk-cotangent-complex}와
\ref{cotangent-lemma-localize}에서 곧바로 따른다
...
보조정리 \ref{cotangent-lemma-triangle-ringed-spaces}의 완전 삼각형에 의해
...
$$
(보조정리 \ref{cotangent-lemma-stalk-cotangent-complex})
가 영임을 보이면 충분하며, 이는 보조정리 \ref{cotangent-lemma-when-zero}에서 따른다.
\end{proof}
...
보조정리 \ref{cotangent-lemma-scheme-over-ring}에 따라
$L_{X/\Spec(\Lambda)} = L_{X/\Lambda}$로 쓴다.
이 절에서는 보조정리 \ref{cotangent-lemma-compute-cotangent-complex}와 비슷하게
...
$$
$U$ 위의 섬유 범주는 절 \ref{cotangent-section-compute-L-pi-shriek}에서 도입한
...
더욱이 주의 \ref{cotangent-remark-resolution}에서 논의했듯이,
...
\medskip\noindent
주의 \ref{cotangent-remark-compute-L-pi-shriek}에 의해 대상
$L\pi_!(\Omega_{\mathcal{O}/\underline{\Lambda}}
\otimes_\mathcal{O} \underline{\mathcal{O}}_X)$
...
$$
여기서는 주의 \ref{cotangent-remark-compute-L-pi-shriek}의 표기를 사용했다.
이제 주의 \ref{cotangent-remark-map-sections-over-U}에 의해
$L\pi_!(\Omega_{\mathcal{O}/\underline{\Lambda}}
\otimes_\mathcal{O} \underline{\mathcal{O}}_X)$
는 $X$ 위의 환 준동형 $\underline{\Lambda} \to \mathcal{O}_X$의
코탄젠트 복합체를 계산한다. 보조정리 \ref{cotangent-lemma-scheme-over-ring}에 의해
이는 바로 원하는 결과이다.
\end{proof}
...
및 정의 \ref{cotangent-definition-cotangent-complex-morphism-ringed-topoi} 참조).
\end{definition}

\noindent
특히 절 \ref{cotangent-section-cotangent-morphism-ringed-topoi}의 결과는
...
(보조정리 \ref{cotangent-lemma-pullback-cotangent-morphism-topoi}),
...
정의 \ref{cotangent-definition-cotangent-morphism-schemes}에 나오는 복합체이다.
\end{lemma}
...
따라서 절 \ref{cotangent-section-cotangent-complex}에서 논의한 함자성과
보조정리 \ref{cotangent-lemma-pullback-cotangent-morphism-topoi}에 의해 표준 사상
$$
...
줄기를 계산하기 위해 보조정리 \ref{cotangent-lemma-stalk-cotangent-complex}를
...
따라서 결과는 보조정리 \ref{cotangent-lemma-cotangent-complex-henselization}에서 따른다.
\end{proof}
...
보조정리 \ref{cotangent-lemma-triangle-ringed-topoi}의 완전 삼각형에 의해
$L_q = 0$임을 보이면 충분하다. 이를 위해서는
(공간의 성질, 정리
\ref{spaces-properties-theorem-exactness-stalks})
...
(공간의 성질, 보조정리
\ref{spaces-properties-lemma-describe-etale-local-ring})
이는 보조정리 \ref{cotangent-lemma-when-zero}에서 따른다.
\end{proof}
...
보조정리 \ref{cotangent-lemma-space-over-ring}에 따라
$L_{X/\Spec(\Lambda)} = L_{X/\Lambda}$로 쓴다.
이 절에서는 보조정리 \ref{cotangent-lemma-compute-cotangent-complex}와 비슷하게
...
\medskip\noindent
주의 \ref{cotangent-remark-compute-L-pi-shriek-spaces}에 의해 대상
$L\pi_!(\Omega_{\mathcal{O}/\underline{\Lambda}}
\otimes_\mathcal{O} \underline{\mathcal{O}}_X)$
는 다음 전층의 복합체의 층화로 계산된다.
$$
...
L_{\mathcal{O}_X(U)/\Lambda}
$$
여기서는 주의 \ref{cotangent-remark-compute-L-pi-shriek-spaces}의 표기를 사용했다.
이제 주의 \ref{cotangent-remark-map-sections-over-U}에 의해
$L\pi_!(\Omega_{\mathcal{O}/\underline{\Lambda}}
\otimes_\mathcal{O} \underline{\mathcal{O}}_X)$
는 $X_\etale$ 위의 환 준동형 $\underline{\Lambda} \to \mathcal{O}_X$의
코탄젠트 복합체를 계산한다. 보조정리 \ref{cotangent-lemma-space-over-ring}에 의해
이는 바로 원하는 결과이다.
\end{proof}
...
보조정리 \ref{cotangent-lemma-triangle-ringed-topoi}의 기본 삼각형이다.
...
(\ref{cotangent-equation-fibre-product})의 대상 $E$는 영이다. 이 경우
$$
...
보조정리 \ref{cotangent-lemma-compare-spaces-schemes}와 공간의 유도 범주, 보조정리
\ref{spaces-perfect-lemma-tor-independent}에 의해 스킴의 경우로 환원된다.
적절한 아핀 열린부분을 택하면 아핀 스킴의 경우로 환원된다.
보조정리 \ref{cotangent-lemma-morphism-affine-schemes}를 사용하면 환의 텐서곱의
경우, 즉 보조정리 \ref{cotangent-lemma-tensor-product-tor-independent}로 환원된다.
\end{proof}
...
$S$를 스킴이라 하자. $X \to B$와 $Y \to B$를 $S$ 위의 대수공간의
사상이라 하자. (\ref{cotangent-equation-fibre-product})의 대상 $E$는
...
보조정리 \ref{cotangent-lemma-stalk-cotangent-complex}와
\ref{cotangent-lemma-pullback-cotangent-morphism-topoi}를 참조하라.
$C$는 $A \otimes_R B$의 헨젤화임에 주의하자.
$L_{C/R} = L_{A \otimes_R B/R} \otimes_{A \otimes_R B} C$
는 절 \ref{cotangent-section-localization}의 결과에서 따른다.
따라서 이 기하점에서 $E$의 줄기는 사상
$L_{A/R} \otimes C \to L_{A \otimes_R B/R} \otimes C$의 사상뿔이다.
그러므로 보조정리의 결과는 환의 경우, 즉 보조정리
\ref{cotangent-lemma-tensor-product}에서 따른다.
\end{proof}
